...
*/
void Novel::display() {
  std::string filmadapt = "It has";
  std::string digital = "It is";
  if(!(hasFilmAdaptation())){
    filmadapt = "It does not have";
  }
  if (!(isDigital()))
  {
    digital = "It is not";
  ...
  std::cout << getTitle() << " is written by " << getAuthor() << ". Genre: " << genre_ << ". " << filmadapt << " a film adaptation. Page Count: " << getPageCount() << ". " << digital << " available digitally.\n";
}
